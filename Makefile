...
MEMMAPPER_CXXFLAGS = -I. `$(WX_CONFIG) --cxxflags $(WX_CONFIG_FLAGS)` $(CPPFLAGS) \
	$(CXXFLAGS)
...
$(PROGNAME): $(CODEOBJECTS)
	$(CXX) -o $@ $(CODEOBJECTS)   $(LDFLAGS)  `$(WX_CONFIG) $(WX_CONFIG_FLAGS) --libs core,base` -no-pie

%.o: %.cpp
	$(CXX) -c -o $@ $(MEMMAPPER_CXXFLAGS) $(CPPDEPS) $<
